...
public class Step09SubflowViaPublishSubscribeTest {
    ...
    public IntegrationFlow classify() {
      return flow -> flow
          ...
          .publishSubscribeChannel(subscription -> subscription
              .subscribe(multipleOfThreeFlow())
              .subscribe(remainderIsOneFlow())
              .subscribe(remainderIsTwoFlow()));
    }

    private IntegrationFlow multipleOfThreeFlow() {
      return flow -> flow
          .filter(Integer.class, this::isMultipleOfThree)
          .channel("multipleOfThreeChannel");
    }

    private IntegrationFlow remainderIsOneFlow() {
      return flow -> flow
          .filter(this::hasRemainderOne)
          .channel("remainderIsOneChannel");
    }

    private IntegrationFlow remainderIsTwoFlow() {
      return flow -> flow
          .filter(Integer.class, this::hasRemainderTwo)
